...
function getMoons()
    ...
    for _, v in pairs(tMoons) do table.insert(aMoons, v); end
    table.sort(aMoons, function(a, b)
        return a.getChild("period").getValue() < b.getChild("period").getValue()
    end);
    return aMoons;
end
...
function getEpochDay(nYear, nMonths)
    ...
    for nCurrentYear = 0, nYear - 1 do
        for nCurrentMonth = 1, nMonths do
            epoch = epoch +
                        CalendarManager.getDaysInMonth(nCurrentMonth,
                                                       nCurrentYear);
        end
    end
    return epoch;
